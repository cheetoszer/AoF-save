...
class Ronin_l(Animation.animate_sprite):
    
    def __init__(self, Game, size):
        super().__init__("Ronin_l", (230,230))
        self.max_hp = 400
        self.hp = 400
    ...
    def update_hp_bar(self, surface):
        ...
        bar_position = [self.rect.x+50, self.rect.y+40, self.hp/3, 5]
        back_bar_position = [self.rect.x+50, self.rect.y +40, self.max_hp/3, 5]
    ...
    def attack_king(self):
        current_time = pygame.time.get_ticks()
        for king_r in self.Game.check_collision(self, self.Game.all_kings_r):
            if current_time - self.last_attack_time >= self.attack_speed:
                king_r.damage(self.attack_value)
                self.last_attack_time = current_time
    ...
    def death(self):
        if self.hp <= 0:
            self.alive = False
            self.kill()
            self.Game.group_base_r.sprites()[0].add_gold_ronin()
            self.Game.group_base_r.sprites()[0].add_xp_ronin()
            return True
