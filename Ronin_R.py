...
class Ronin_r(Animation.animate_sprite):
    
    def __init__(self, Game, size):
        super().__init__("Ronin_r", (230,230))
        self.max_hp = 400
        self.hp = 400
    ...
    def update_hp_bar(self, surface):
        ...
        back_bar_position = [self.rect.x+50, self.rect.y +40, 3/self.max_hp, 5]
    ...
    def death(self):
        if self.hp <= 0:
            self.alive = False
            self.kill()
            self.Game.group_base_l.sprites()[0].add_gold_ronin()
            self.Game.group_base_l.sprites()[0].add_xp_ronin()
            return True
