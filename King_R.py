...
class King_r(Animation.animate_sprite):
    
    def __init__(self, Game, size):
        super().__init__("King_r", (230,230))
        self.max_hp = 300
        self.hp = 300
    ...
    def update_hp_bar(self, surface):
        ...
        bar_position = [self.rect.x+70, self.rect.y+40, self.hp/3, 5]
        back_bar_position = [self.rect.x+70, self.rect.y +40, self.max_hp/3, 5]
    ...
    def death(self):
        if self.hp <= 0:
            self.alive = False
            self.kill()
            self.Game.group_base_l.sprites()[0].add_gold_king()
            self.Game.group_base_l.sprites()[0].add_xp_king()
            return True
